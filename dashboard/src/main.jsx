...
function aggregateMonthly(points) {
  ...
  (points || []).forEach((point) => {
    ...
    const bucket = buckets.get(key) || { key, values: [], observed: 0, missing: 0 };
    if (point.value == null || !Number.isFinite(Number(point.value))) bucket.missing += 1;
    else { bucket.values.push(Number(point.value)); bucket.observed += 1; }
  ...
  const months = [];
  for (const date = new Date(first); date <= last; date.setUTCMonth(date.getUTCMonth() + 1)) {
    const key = `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, "0")}`;
    const bucket = buckets.get(key) || { key, values: [], observed: 0, missing: 0 };
    months.push({
      ...bucket,
      date: `${date.toLocaleString("en-US", { month: "short", timeZone: "UTC" })} ${date.getUTCFullYear()}`,
      value: bucket.observed ? bucket.values.reduce((sum, value) => sum + value, 0) / bucket.observed : null,
    });
  }
  return months;
}
...
function peakObserved(points) {
  const values = (points || []).filter((point) => point.value != null).map((point) => Number(point.value)).filter(Number.isFinite);
  return values.length ? Math.max(...values) : null;
}

function historyStats(points) {
  const observed = (points || []).filter((point) => point.value != null && Number.isFinite(Number(point.value)));
...
function LineChart({ points, feederPoints }) {
  ...
  const chart = useMemo(() => {
    const sourcePoints = mode === "monthly" ? aggregateMonthly(points) : points || [];
    const sourceFeederPoints = mode === "monthly" ? aggregateMonthly(feederPoints) : feederPoints || [];
    const dated = sourcePoints.map((point, index) => ({ ...point, index, value: point.value == null ? Number.NaN : Number(point.value) }));
    const feederDated = sourceFeederPoints.map((point, index) => ({ ...point, index, value: point.value == null ? Number.NaN : Number(point.value) }));
    const valid = dated.filter((point) => Number.isFinite(point.value));
    if (!valid.length) return null;
    const values = valid.map((point) => point.value).concat(feederDated.filter((point) => Number.isFinite(point.value)).map((point) => point.value));
    ...
    return {
      ...
      feederPath: feederDated.length ? toChartPath(feederDated, min, range, denominator) : "",
      firstDate: dated[0]?.date,
      lastDate: dated.at(-1)?.date,
      labels: dated.length > 7 ? dated.filter((_, index) => index === 0 || index === dated.length - 1 || index % Math.ceil(dated.length / 6) === 0) : dated,
      points: dated,
    };
  }, [points, feederPoints, mode]);
  if (!chart) return <p className="history-empty">{chartError || "Insufficient meter history for trend visualization."}</p>;
  const denominator = Math.max(chart.points.length - 1, 1);
  return <><section className="signals-section feeder-context"><h3>Feeder Context</h3>{feederContext?.mapping_available ? <><p>Verified feeder metadata; supporting context only, not a model input.</p><div className="available-signals"><div><span>Feeder</span><strong>{feederContext.feeder_id}</strong></div><div><span>Transformer</span><strong>{feederContext.transformer_id || "—"}</strong></div><div><span>Area</span><strong>{feederContext.area || "—"}</strong></div><div><span>Consumer average</span><strong>{feederContext.consumer_average?.toFixed(2) ?? "—"} kWh</strong></div><div><span>Feeder average</span><strong>{feederContext.feeder_average?.toFixed(2) ?? "—"} kWh</strong></div><div><span>Deviation</span><strong>{feederContext.deviation_percent == null ? "—" : `${feederContext.deviation_percent.toFixed(1)}%`}</strong></div></div></> : <><p>Feeder mapping unavailable</p><div className="history-empty">Feeder-level comparison requires verified feeder metadata.</div></>}</section><div className="history-summary history-insights"><div><span>Latest Available Usage</span><strong>{stats.latest ? formatChartValue(stats.latest.value) : "—"}</strong><small>{stats.latest ? `Date: ${formatHistoryDate(stats.latest.date)}` : "No observed date"}</small></div><div><span>Mean</span><strong>{stats.mean == null ? "—" : formatChartValue(stats.mean)}</strong></div><div><span>Peak</span><strong>{stats.peak == null ? "—" : formatChartValue(stats.peak)}</strong></div><div><span>Observed</span><strong>{stats.observed}</strong></div><div><span>Missing</span><strong>{stats.missing}</strong></div></div><div className="history-period"><strong>Historical period:</strong> {formatHistoryDate(stats.firstDate)} – {formatHistoryDate(stats.lastDate)}</div><div className="history-controls" role="group" aria-label="Consumption history view"><button className={mode === "monthly" ? "active" : ""} onClick={() => setMode("monthly")}>Monthly Trend</button><button className={mode === "raw" ? "active" : ""} onClick={() => setMode("raw")}>Raw Readings</button></div><div className="history-chart"><span className="chart-y-label">Reported consumption</span><div className="chart-scale"><span>{chart.max.toFixed(1)}</span><span>{chart.min.toFixed(1)}</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={`${mode === "monthly" ? "Monthly" : "Raw"} reported consumption history`}><path className="chart-grid" d="M0 16H100M0 54H100M0 92H100" />{chart.feederPath && <path className="feeder-path" d={chart.feederPath} />}<path className="consumption-path" d={chart.path} />{chart.points.map((point) => Number.isFinite(point.value) && <circle key={point.key} className="consumption-point" cx={(point.index / denominator) * 100} cy={92 - ((point.value - chart.min) / (chart.max - chart.min || 1)) * 76} r="1.2"><title>{mode === "monthly" ? `${point.date}\nAverage reported consumption: ${formatChartValue(point.value)}\nObserved readings: ${point.observed}` : `${point.date}\nReported consumption: ${formatChartValue(point.value)}`}</title></circle>)}</svg><div className="chart-axis-label">{mode === "monthly" ? "Month" : "Date"}</div><div className="chart-dates">{chart.labels.map((point) => <span key={point.key}>{point.date}</span>)}</div></div></>;
}
...
function CalibrationChart({ bins }) {
  ...
  return <div className="calibration-chart"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Calibration reliability diagram"><path className="chart-grid" d="M0 0V100H100" /><path className="calibration-reference" d="M0,100 L100,0" /><path className="calibration-path" d={path} /></svg><span className="calibration-y-label">Observed positive rate</span><div className="calibration-axis"><span>Predicted risk</span><span>Ideal calibration: dashed diagonal</span></div></div>;
}
...
function App() {
  ...
  useEffect(() => {
    ...
    api(`/api/inspections?${parameters}`).then((data) => { setItems(shuffled(data.items)); setTotalResults(data.total); setError(""); }).catch(() => { setItems([]); setTotalResults(0); setError("Live risk data is temporarily unavailable. Check that the local API is running."); }).finally(() => setQueueLoading(false));
  ...
  useEffect(() => {
    ...
    if (debouncedConsumerQuery) countParameters.set("query", debouncedConsumerQuery);
    api(`/api/inspections?${countParameters}`)
      .then((countData) => {
        if (consumerPage === 0) {
          const sampleSizes = { HIGH: 8, MEDIUM: 8, LOW: 9 };
          return Promise.all(Object.entries(sampleSizes).map(([level, limit]) => {
            const parameters = new URLSearchParams({ limit: String(limit), offset: "0", priority: level, randomize: "true" });
            if (debouncedConsumerQuery) parameters.set("query", debouncedConsumerQuery);
            return api(`/api/inspections?${parameters}`);
          })).then((groups) => {
            setConsumerItems(shuffled(groups.flatMap((group) => group.items)).slice(0, PAGE_SIZE));
            setConsumerTotal(countData.total);
          });
        }
        const maxOffset = Math.max(countData.total - PAGE_SIZE, 0);
        const offset = consumerPage === 0 ? Math.floor(Math.random() * (maxOffset + 1)) : consumerPage * PAGE_SIZE;
        const parameters = new URLSearchParams({ limit: String(PAGE_SIZE), offset: String(offset), randomize: "true" });
        if (debouncedConsumerQuery) parameters.set("query", debouncedConsumerQuery);
        return api(`/api/inspections?${parameters}`).then((data) => { setConsumerItems(shuffled(data.items)); setConsumerTotal(data.total); });
      })
  ...
  useEffect(() => {
    ...
    api(`/api/customers/${selected.CONS_NO}/consumption`)
      .then((data) => setConsumption({ ...data, points: data.points.map((point, index) => ({ ...point, key: `${point.date}-${index}` })) }))
      .catch((requestError) => {
        console.error("Consumption history request failed", requestError);
        setConsumption({ points: [{ error: "Meter history could not be loaded. Reopen this profile to retry." }], feeder_points: [], summary: { observed_readings: 0, missing_readings: 0, mean_consumption: null } });
      })
  ...
  useEffect(() => {
    const sections = [
      ...
      ["Consumers", "consumers"],
      ["Analytics", "analytics"],
    ].map(([label, id]) => ({ label, element: document.getElementById(id) })).filter((section) => section.element);
    const visible = new Set();
    const updateActiveSection = () => {
      const current = [...visible]
        .sort((left, right) => left.element.getBoundingClientRect().top - right.element.getBoundingClientRect().top)[0];
